...
const MapComponent: React.FC<{ onNeighborhoodClick: (neighborhood: NeighborhoodProps) => void }> = ({ onNeighborhoodClick }) => {
  ...
  const onEachFeature = (feature: GeoJSON.Feature, layer: L.Layer) => {
    const properties = feature.properties as NeighborhoodProps;
    console.log(properties)
    if (properties && properties.nhood) {
      const popupContent = `
        <div>
          <h3>${properties.nhood}</h3>
          <!-- Add more content here based on properties -->
        </div>
      `;
  ...
  return (
    <MapContainer center={[37.7749, -122.4194]} zoom={12} style={{ height: "100vh", width: "100%" }}>
      ...
      />
      {geojson && (
        <GeoJSON data={geojson} {...geoJsonStyle} />
      )}
    </MapContainer>
  );
};
